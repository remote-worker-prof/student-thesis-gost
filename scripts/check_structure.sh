...
# Приложения должны маркироваться в формате "Приложение А", "Приложение Б", ...
# Для устойчивости в CI используем фиксированный токен первого приложения.
if ! grep -Fq 'Приложение А' "$text_file"; then
  missing+=("Приложение А")
fi
